      ...
      <div className="min-h-screen bg-gray-100 py-10 px-4 flex flex-col items-center">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            直近1週間の {totalMatches.toLocaleString()} 試合の統計データから重み付き集計
          </p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">カード</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-20 hidden sm:table-cell">レアリティ</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-24 hidden sm:table-cell">タイプ</th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-16 hidden sm:table-cell">コスト</th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase w-20">使用ポイント</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {ranking.map(entry => {
                    const tier = ORIGINALITY_TIERS.find(
                      t => entry.rank >= t.startRank && (t.endRank === null || entry.rank <= t.endRank)
                    );
    ...
    <>
      ...
      <tr className="hover:bg-gray-50">
        ...
        <td className="px-3 py-2 hidden sm:table-cell">
          {entry.rarity && (
            <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded ${rarityClasses[entry.rarity] ?? ''}`}>
        ...
        <td className="px-3 py-2 hidden sm:table-cell">
          {entry.rarity && (
            <span className={`inline-flex items-center justify-center w-7 h-7 rounded text-xs font-bold ${colorClasses[entry.color] ?? 'bg-gray-400 text-white'}`}>
              {entry.cost}
            </span>
          )}
        </td>
