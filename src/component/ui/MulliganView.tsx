...
export const MulliganView: React.FC = () => {
  ...
  // handleNoの参照を更新（タイマー等のクロージャ対策）
  useEffect(() => {
    handleNoRef.current = handleNo;
  }, [handleNo]);

  // オートエンド設定が有効な場合、タイムアウト時に自動確定
  useEffect(() => {
  ...
  return (
    <div className="absolute w-full flex flex-col items-center bg-black/70 p-2.5 rounded-lg z-10 -top-[150px]">
      ...
      <div className="flex gap-5">
        <button
          className="bg-blue-600 hover:bg-blue-500 text-white py-2 px-8 border-0 rounded cursor-pointer transition-colors font-bold"
          onClick={handleYes}
        >
          はい
        </button>
        <button
          className="bg-red-600 hover:bg-red-500 text-white py-2 px-8 border-0 rounded cursor-pointer transition-colors font-bold"
          onClick={handleNo}
        >
          いいえ
        </button>
