...
const UnitViewComponent = ({ unit, isOwnUnit = false }: UnitViewProps) => {
  ...
  // 繝ｦ繝九ャ繝医・菴咲ｽｮ諠・ｱ繧偵さ繝ｳ繝・く繧ｹ繝医↓逋ｻ骭ｲ
  useEffect(() => {
  ...
  const hasVirusSpecies = unitMaster?.species?.includes('繧ｦ繧｣繝ｫ繧ｹ') || false;
  const hasEvolutionBan =
    unit.delta?.some(delta => 'name' in delta.effect && delta.effect.name === '騾ｲ蛹也ｦ∵ｭ｢') || false;
  ...
  // 繧｢繧ｿ繝・け蜿ｯ閭ｽ譚｡莉ｶ縺ｮ蛻､螳・
  const hasActionRestriction = hasKeyword('陦悟虚蛻ｶ髯・);
  const hasAttackBan = hasKeyword('謾ｻ謦・ｦ∵ｭ｢');
  const hasSilence = hasKeyword('豐磯ｻ・);
  const canAttack = unit.active && !hasActionRestriction && (!hasAttackBan || hasSilence);

  // 謦､騾蜿ｯ閭ｽ譚｡莉ｶ縺ｮ蛻､螳・
  const hasWithdrawBan = hasKeyword('謦､騾遖∵ｭ｢');
      ...
      <div className="relative w-32 h-32 unit-wrapper">
            ...
            <UnitActionButtons
              ...
              canBoot={
                unit.hasBootAbility === true
                  ? unit.isBooted ||
                    unit.delta?.some(
                      delta => delta.effect.type === 'keyword' && delta.effect.name === '豐磯ｻ・
                    )
                    ? false
                    : true
                  : undefined
        ...
        >
          ...
          <div className="absolute inset-0 z-10 pointer-events-none">
            {/* 譌｢蟄倥・蜉ｹ譫懃匱蜍輔い繝九Γ繝ｼ繧ｷ繝ｧ繝ｳ・磯聞譁ｹ蠖｢繝代ち繝ｼ繝ｳ・・- unit-selection/context逕ｱ譚･ */}
            <UnitIconEffect
              show={animationUnit === unit.id}
              onComplete={() => setAnimationUnit(undefined)}
            />

            {/* 譁ｰ縺励＞驕ｸ謚槭お繝輔ぉ繧ｯ繝茨ｼ亥・蠖｢諡｡謨｣・・- select-effect/context逕ｱ譚･ */}
            {targetUnitIds.includes(unit.id) && (
              <SelectEffect
                unitId={unit.id}
                onComplete={() => {
                  // 螳御ｺ・ワ繝ｳ繝峨Λ縺ｧ縺薙・繧｢繝九Γ繝ｼ繧ｷ繝ｧ繝ｳ蟇ｾ雎｡縺ｮID繧偵Μ繧ｻ繝・ヨ
                  removeTargetUnit(unit.id);
                }}
              />
            )}

            {/* 繧ｪ繝ｼ繝舌・繧ｯ繝ｭ繝・け繧ｨ繝輔ぉ繧ｯ繝・- overclock-effect/context逕ｱ譚･ */}
            {useOverclockEffect().activeUnits.includes(unit.id) && (
              <OverclockEffect
                unitId={unit.id}
                onComplete={() => {
                  /* 螳御ｺ・ワ繝ｳ繝峨Λ縺ｯ蜀・Κ縺ｧ繧ｳ繝ｳ繝・く繧ｹ繝医ｒ繝ｪ繧ｻ繝・ヨ */
                }}
              />
            )}

            {/* 繧ｹ繝・・繧ｿ繧ｹ螟画峩繧ｨ繝輔ぉ繧ｯ繝・- status-change/context逕ｱ譚･ */}
            {useStatusChange()
          ...
          {/* 騾ｲ蛹悶Θ繝九ャ繝医ｒ 繝峨Λ繝・げ縺励※荳翫↓驥阪・縺ｦ縺・ｋ髢薙・縺ｿ陦ｨ遉ｺ縺輔○繧・*/}
