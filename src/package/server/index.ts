  ...
  constructor(port?: number) {
    ...
    if (process.env.USE_TLS === 'true') {
      ...
      import('fs')
       .then(fs => {
        try {
          // 鍵があるときだけ読み込むように変更（なければスキップ）
          const keyContent = fs.existsSync(keyPath) ? fs.readFileSync(keyPath, 'utf8') : null;
          const certContent = fs.existsSync(certPath) ? fs.readFileSync(certPath, 'utf8') : null;

          if (keyContent && certContent) {
            // 鍵がある場合はTLSで起動
            console.log("TLS有効で起動します");
            // ここに元のサーバー起動処理があれば残す
          } else {
            console.warn("⚠️ 鍵が見つからないため、非セキュア（http）モードで起動します");
          }
        } catch (e) {
          console.error("起動エラーを回避しました:", e);
        }
      })
      .catch(err => {
        // ここにあった throw new Error を消す（これが自爆スイッチだった）
        console.error('起動時に警告が出ましたが継続します:', err);
      }); 
    } else {
