  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {firstPlayerIndex === playerIndex && (
        <span className="text-xs font-medium text-blue-600">蜈・/span>
      )}
      {firstPlayerIndex !== null && firstPlayerIndex !== playerIndex && (
        <span className="text-xs font-medium text-red-600">蠕・/span>
      )}
      {player.id ? (
        <Link
        ...
        <span className="text-sm text-gray-900">{player.name || '荳肴・'}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900">蜈ｨ蟇ｾ謌ｦ螻･豁ｴ ({total}莉ｶ)</h3>
      </div>
