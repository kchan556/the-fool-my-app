        ...
        <details className="w-full max-w-6xl p-4">
          ...
          <div className="w-full max-w-6xl mt-5">
            ...
            <div className="px-4 mb-4 flex flex-wrap items-center gap-2">
              <span className="text-sm text-gray-300">荳ｦ縺ｳ鬆・</span>
              {[
                { value: 'id' as const, label: '繧ｫ繝ｼ繝迂D' },
                { value: 'color' as const, label: '螻樊ｧ' },
                { value: 'cost' as const, label: '繧ｳ繧ｹ繝・ },
                { value: 'rank' as const, label: '蛻ｩ逕ｨ繝ｩ繝ｳ繧ｭ繝ｳ繧ｰ' },
              ].map(option => (
                <label
                  key={option.value}
            ...
            {/* 蜉ｹ譫懷ｮ溯｣・ｸ医∩縺ｮ縺ｿ陦ｨ遉ｺ繝√ぉ繝・け繝懊ャ繧ｯ繧ｹ */}
            ...
            <div className="px-4 my-6">
              <input
                type="text"
                placeholder="繧ｫ繝ｼ繝牙錐繧・柑譫懊ユ繧ｭ繧ｹ繝医〒讀懃ｴ｢"
            ...
            <div className="px-4 mb-6 flex flex-wrap gap-6">
              ...
              <div className="filter-group">
                <h3 className="font-bold mb-2">繝ｬ繧｢繝ｪ繝・ぅ</h3>
              ...
              {/* 螻樊ｧ繝輔ぅ繝ｫ繧ｿ */}
              <div className="filter-group">
                <h3 className="font-bold mb-2">螻樊ｧ</h3>
                <div className="flex flex-wrap gap-2">
              ...
              {/* OP (Originality) 繝輔ぅ繝ｫ繧ｿ */}
              <div className="filter-group">
                <h3 className="font-bold mb-2">繧ｪ繝ｪ繧ｸ繝翫Μ繝・ぅ</h3>
                <div className="flex flex-wrap gap-2">
              ...
              {/* 繝舌・繧ｸ繝ｧ繝ｳ繝輔ぅ繝ｫ繧ｿ */}
              <div className="filter-group">
                <h3 className="font-bold mb-2">繝舌・繧ｸ繝ｧ繝ｳ</h3>
            ...
            <div className="px-4 mb-6">
              <h3 className="font-bold mb-2">繧ｳ繧ｹ繝・/h3>
              <div className="flex flex-wrap gap-2">
            ...
            {/* 遞ｮ譌上ヵ繧｣繝ｫ繧ｿ */}
            <div className="px-4 mb-6">
              <h3 className="font-bold mb-2">遞ｮ譌・/h3>
            ...
            {/* 繧ｿ繧､繝励ヵ繧｣繝ｫ繧ｿ */}
            <div className="px-4 mb-6">
              <h3 className="font-bold mb-2">繧ｫ繝ｼ繝峨ち繧､繝・/h3>
              <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto border p-2 rounded">
                {[
                  { name: '繝ｦ繝九ャ繝・, value: 'unit' },
                  { name: '繝医Μ繧ｬ繝ｼ', value: 'trigger' },
                  { name: '繧､繝ｳ繧ｿ繝ｼ繧ｻ繝励ヨ', value: 'intercept' },
                  { name: '騾ｲ蛹・, value: 'advanced_unit' },
                ].map(type => (
                  <button
                    key={type.value}
...
export const DeckBuilder = ({ implementedIds, opMap }: DeckBuilderProps) => {
  ...
  const handleSelectJoker = useCallback(
    (catalogId: string) => {
      if (jokers.length < 2) {
        ...
        alert('JOKER縺ｯ譛螟ｧ2譫壹∪縺ｧ縺ｧ縺吶・);
      }
    },
    [jokers]
  );

  // JOKER霑ｽ蜉繝懊ち繝ｳ縺ｮ繝上Φ繝峨Λ繝ｼ
  const handleOpenJokerDialog = useCallback(() => {
    if (jokers.length >= 2) {
      alert('JOKER縺ｯ譛螟ｧ2譫壹∪縺ｧ縺ｧ縺吶よ里蟄倥・JOKER繧偵け繝ｪ繝・け縺励※蜑企勁縺励※縺上□縺輔＞縲・);
    } else {
  ...
  // 蜷悟錐繧ｫ繝ｼ繝峨・rank諠・ｱ繧貞叙蠕励☆繧九◆繧√・繝槭ャ繝励ｒ讒狗ｯ・
  ...
  const filteredCatalogs = useMemo(() => {
    ...
    catalogs.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'rank': {
          // rank縺後↑縺・ｴ蜷医・蜷悟錐繧ｫ繝ｼ繝峨・rank繧貞盾辣ｧ
    ...
    async (title: string, isMainDeck: boolean) => {
      if (deck.length === 40) {
        try {
          ...
          alert(
            `繝・ャ繧ｭ縲・{title}縲阪′菫晏ｭ倥＆繧後∪縺励◆縲・{isMainDeck ? '・医Γ繧､繝ｳ繝・ャ繧ｭ縺ｫ險ｭ螳壹＆繧後∪縺励◆・・ : ''}`
          );
        } catch (error) {
          console.error('繝・ャ繧ｭ菫晏ｭ倥お繝ｩ繝ｼ:', error);
          alert('繝・ャ繧ｭ縺ｮ菫晏ｭ倥↓螟ｱ謨励＠縺ｾ縺励◆縲・);
        }
      }
    },
  ...
  const sortDeck = useCallback(() => {
    setDeck(prevDeck => {
      ...
      typeCategories.forEach(typeCategory => {
        ...
        // 繧ｫ繝ｩ繝ｼ鬆・↓荳ｦ縺ｹ譖ｿ縺医※縲∝推繧ｫ繝ｩ繝ｼ蜀・〒catalogId繧偵た繝ｼ繝・
