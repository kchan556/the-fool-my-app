    ...
    <div className="max-w-7xl mx-auto p-4">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-400">
          ...
          <span>{deck.cards.length}譫・/span>
          {(deck.jokers?.length ?? 0) < 2 && <span className="text-red-400">JOKER縺ｪ縺・/span>}
          {isPublic && <span className="text-green-400">蜈ｬ髢倶ｸｭ</span>}
        </div>
        <div className="mt-3">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="mt-6 flex justify-center gap-2">
          <RichButton onClick={() => router.back()}>髢峨§繧・/RichButton>
          {isOwner && (
            <RichButton colorScheme="blue" onClick={handleTogglePublic}>
              {isPublic ? '繝・ャ繧ｭ繧帝撼蜈ｬ髢九↓縺吶ｋ' : '繝・ャ繧ｭ繧貞・髢九☆繧・}
            </RichButton>
          )}
          {!isOwner && (
